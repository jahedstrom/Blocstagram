...
- (void)viewWillLayoutSubviews {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        horizontalLine.frame = CGRectMake(0, (i * thirdOfWidth) + CGRectGetMaxY(self.topView.frame), width, 0.5); // 0.5 makes the view a line
        
        CGRect verticalFrame = CGRectMake(i * thirdOfWidth, CGRectGetMaxY(self.topView.frame), 0.5, width);
        
        if (i == 3) {
            verticalFrame.origin.x -= 0.5;  // backs off the last vertical line by 1/2 point from the right side of the screen, not sure why?
        }
